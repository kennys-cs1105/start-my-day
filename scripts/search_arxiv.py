...
def main():
    ...
    parser.add_argument('--arxiv-xml', type=str, default=None,
                        help='Load recent papers from local arXiv Atom XML (skip arXiv API fetch)')
    ...
    if focus_keywords:
        ...
        logger.info("=" * 70)
        if args.arxiv_xml:
            logger.info("Step 1: Loading recent papers from local XML: %s", args.arxiv_xml)
        else:
            logger.info("Step 1: Searching recent papers (last 30 days) from arXiv")
        logger.info("=" * 70)

        if args.arxiv_xml:
            with open(args.arxiv_xml, 'r', encoding='utf-8') as f:
                recent_papers = parse_arxiv_xml(f.read())
            logger.info("[arXiv] Loaded %d papers from local XML", len(recent_papers))
        else:
